								...
								<div class="card-front">
									<div class="center-wrap">
										...
										@if ($errors->any())
											...
											@endif
											<h4 class="mb-4 pb-3">Digite sua nova senha {{$entidade->name}}</h4>
											<form action="{{route('recSenhaEntidade')}}" method="POST">
												@csrf
         									    @method('PUT') 
												<div class="form-group">	
													<input type="hidden" name="entidade" value="{{$entidade->id}}">
													<input type="password" name="password" class="form-style" placeholder="Informe sua nova senha" id="password" autocomplete="off">
